...
// Função para mostrar os benefícios de um plano como lista
function mostrarBeneficios(beneficios) {
    const beneficioContainer = document.getElementById("beneficioContainer");
    beneficioContainer.style.display = "block";
    beneficioContainer.innerHTML = `<h3>Benefícios:</h3>`;
    
    const ul = document.createElement("ul");  // Cria a lista não ordenada (ul)
    
    beneficios.forEach(beneficio => {
        const li = document.createElement("li");  // Cria cada item de lista (li)
        li.textContent = beneficio;  // Adiciona o texto do benefício
        ul.appendChild(li);  // Adiciona o item à lista
    });

    beneficioContainer.appendChild(ul);  // Adiciona a lista ao container
}
